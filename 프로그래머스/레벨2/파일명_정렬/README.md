...
## 풀이
1. `Name` 클래스를 선언한다.
   1. `index`, `head`, `number`, `tail`를 필드로 가진다.
   2. `Comparable`을 구현해서 정렬 기준을 세운다.
2. `Name`들을 정의할 `names` 배열을 생성한다.
3. `files`를 순회하며 다음을 수행한다.
   1. `file`을 순회하며 `head`, `number`, `tail`을 구한다.
   2. 구한 `head`, `number`, `tail`을 이용해서 `Name`클래스로 만들어 `names`에 저장한다.
4. `names`를 정렬하고 각 `Name`을 `String`으로 변환하여 리턴한다.