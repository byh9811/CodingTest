...
-----------
## 풀이
1. 연산자가 총 3개 이므로 6가지의 경우의 수가 존재한다.
2. 각 경우의 수로 수식을 계산한다.
   1. +, -, *로 expression을 나누고, 피연산자와 연산자를 따로 저장한다.
   2. 연산자 우선순위를 정한다.
   3. 연산자 배열을 순회하며 현재 우선순위인 연산을 수행하고 같은 위치에 그대로 저장한다.
   4. 연산자 인덱스가 i라면, 피연산자 i, i+1과 연산하며 i위치에 저장한다.
3. 수식의 절댓값이 가장 큰 값을 리턴한다.