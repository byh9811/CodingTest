...
-----------
## 풀이
1. `N` x `N`의 char형 배열 `map`을 생성하고 입력받는다.
2. `map`과 같은 크기의 정답을 기록할 배열 `ret`를 생성한다.
3. `map`을 순회하면서 숫자가 나오면 다음을 수행한다.
   1. `ret`의 동일 인덱스값을 `*`로 변경하고 근방의 값이 `*`이 아니면 숫자만큼 더한다.
   2. 만약 더한 값이 10을 넘어갈 경우 `M`으로 표시한다.
4. `ret`을 형식에 맞게 출력한다.